...
- 在主窗口侧边栏实时筛选项目和任务，支持按名称搜索以及“全部 / 执行中”状态切换
...
- 配置自动保存，修改后实时生效
- 新窗口和页面刷新后自动重新注入
- 只修改 Codex 主工作区窗口，不影响桌面宠物及其辅助浮窗
...
随后通过 Chrome DevTools Protocol 识别 Codex 主工作区窗口，为它创建独立样式表并注入共享的布局主题引擎与侧边栏筛选器。桌面宠物的 `avatar-overlay` 页面和 composition surface 会在连接前被排除。引擎使用隔离的 Shadow DOM 生成工具栏、信息栏和筛选控件，不移动 Codex 自己管理的 React 节点；筛选器只给未匹配的原生项目或任务节点添加临时隐藏标记，并监听任务新增、完成和页面重绘。启动器会持续监控主窗口、页面刷新和配置变化。背景图会在本地压缩后转换为 Data URL，不会上传到网络。
...
- 项目不会上传或保存 Codex 对话内容。布局与筛选脚本只在页面内读取当前标题、路径、可见项目/任务名称、运行状态元素以及按钮标签，用于显示任务信息、实时筛选和转发快捷操作；这些数据不会写入配置或日志。
...
测试会验证额度稀疏更新合并、App Server 事件、原始与三种工作台布局、项目/任务名称与执行状态筛选、CDP 注入和桌面宠物窗口排除，结束后主动关闭测试进程。Windows 版可使用 `dotnet build windows/CodexSkinLauncher.Windows.csproj --configuration Release` 编译验证。
